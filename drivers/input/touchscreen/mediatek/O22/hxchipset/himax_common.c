...
/*hs14 code for SR-AL6528A-01-460 by zhangxiongyi at 20220915 start*/
bool usb_detect_flag;
/*hs14 code for SR-AL6528A-01-460 by zhangxiongyi at 20220915 end*/
#endif
...
/*hs14 code for AL6528A-1034 by zhangxiongyi at 20221208 end*/
{
    ...
    result=g_core_fp.fp_chip_self_test_for_tp();
...
static ssize_t hx_proc_getinfo_read(struct file *flip,char __user *buff,size_t size,loff_t *pPos)
{
    char buf[50] = {0};
    int rc = 0;
    struct himax_ic_data *hx_ic_data = ic_data;
    ...
    I("%s\n",__func__);
    /*hs14 code for SR-AL6528A-01-459 by hehaoran5 at 20220928 start*/
    snprintf(buf,sizeof(buf),"IC_NAME:HX83112F_TXD_BOE TOUCH_VER:%02x \n",hx_ic_data->vendor_touch_cfg_ver & 0xFF);
    /*hs14 code for SR-AL6528A-01-459 by hehaoran5 at 20220928 end*/
...
{
    hx_info_proc_entry = proc_create(HX_INFO_PROC_FILE,0666,NULL,&hx_info_proc_fops);
    if (NULL == hx_info_proc_entry) {
        E("%s():Couldn't create proc entry!\n",__func__);
        return -EFAULT;
    } else {
        I("%s():Create proc entry success\n",__func__);
    }
    return 0;
}
...
/*hs14 code for SR-AL6528A-01-463 by zhangxiongyi at 2022091 end*/
...
static void himax_fb_register(struct work_struct *work)
{
    ...
    ts->esd_recovery_notifier.notifier_call = esd_recovery_notifier_callback;
    ret = register_esd_tp_recovery_notifier(&ts->esd_recovery_notifier);
    if (ret) {
        E("register_esd_tp_recovery_notifier fail\n");
    }
...
int himax_enable_gesture_wakeup(void)
{
    struct himax_ts_data *ts = private_ts;
    if (ts ==NULL) {
        return -EFAULT;
...
int himax_disable_gesture_wakeup(void)
{
    struct himax_ts_data *ts = private_ts;
    if (ts ==NULL) {
        return -EFAULT;
...
static struct platform_device hxinfo_device = {
    .name = HXINFO_NAME,
    .id = -1,
};
...
static struct attribute *hx_ito_test_atts[] = {
        &dev_attr_factory_check.attr,
        NULL,
};

static const struct attribute_group hx_ito_test_attribute_group = {
    .attrs = hx_ito_test_atts,
};

static int hx_test_node_init(struct platform_device *tdinfo_device)
{
    int err = 0;
    err = sysfs_create_group(&tdinfo_device->dev.kobj,&hx_ito_test_attribute_group);
    if (0 != err) {
        E("ERROR: ITO node create failed.");
        sysfs_remove_group(&tdinfo_device->dev.kobj,&hx_ito_test_attribute_group);
        return 0;
    } else {
...
static int tp_usb_check_notify_callback(struct notifier_block *this,unsigned long event,void *ptr)
{
...
static void himax_tp_headset_workqueue(struct work_struct *work)
{
    ...
    if (private_ts->suspended == false) {
        ...
        ret = himax_headset_mode_set(private_ts->earphone_enable);
        if (!ret) {
...
static int tp_earphone_check_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
 ...
 */
    ...
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 start*/
    I("%s(): start init ito_test",__func__);
    platform_device_register(&hxinfo_device);
    hx_test_node_init(&hxinfo_device);
    I("%s(): ito_test init success",__func__);
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 end*/

    /*hs14 code for SR-AL6528A-01-457 by hehaoran5 at 20220910 start*/
    ret = sec_cmd_init(&private_ts->sec, himax_commands, himax_get_array_size(), SEC_CLASS_DEVT_TSP);
    if (ret < 0) {
...
/*hs14 code for SR-AL6528A-01-458 by hehaoran5 at 20220913 start*/
#ifdef HQ_D85_BUILD
    himax_enable_gesture_wakeup();
#endif
/*hs14 code for SR-AL6528A-01-458 by hehaoran5 at 20220913 end*/
    ...
    /*hs14 code for SR-AL6528A-01-463 by zhangxiongyi at 202209015 end*/
#if defined(HX_CONTAINER_SPEED_UP)
    ts->ts_int_workqueue =
    ...
    ts->earphone_notif.notifier_call = tp_earphone_check_notifier_callback;
    ret = headset_notifier_register(&ts->earphone_notif);
    if (ret) {
        E("unable to register headset_notifier_register\n");
    }
    /*hs14 code for AL6528ADEU-977 by zhangxiongyi at 20221019 end*/
    /*hs14 code for SR-AL6528A-01-460 by zhangxiongyi at 20221010 start*/
    ts->tp_usb_notify.notifier_call = tp_usb_check_notify_callback;
    ret = register_usb_check_notifier(&ts->tp_usb_notify);
    if (ret) {
        E("%s:tp_usb_notify register fail\n", __func__);
    }
    /*hs14 code for SR-AL6528A-01-460 by zhangxiongyi at 20221010 end*/
    return 0;
...
#if defined(HX_CONFIG_FB)
    /*hs14 code for SR-AL6528A-01-460 by zhangxiongyi at 20221010 start*/
    unregister_usb_check_notifier(&ts->tp_usb_notify);
    /*hs14 code for SR-AL6528A-01-460 by zhangxiongyi at 20221010 end*/
    /*hs14 code for AL6528ADEU-977 by zhangxiongyi at 20221019 start*/
    cancel_work_sync(&ts->headset_work_queue);
    headset_notifier_unregister(&ts->earphone_notif);
    /*hs14 code for AL6528ADEU-977 by zhangxiongyi at 20221019 end*/
    /*hs14 code for AL6528A-213 by hehaoran5 at 20221002 start*/
...
END:
    ...
    if (ic_data != NULL) {
        I("IC_NAME:HX83112F_TXD_BOE TOUCH_VER:%02x\n",ic_data->vendor_touch_cfg_ver & 0xFF);
    }
